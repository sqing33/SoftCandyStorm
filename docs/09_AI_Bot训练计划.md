...
针对 seed `63405` 这种 opening 与 mid-window 同时出现 boundary/path retention debt 的 lane，使用更窄的 `opening-mid-boundary-retention`，训练前必须先固定 `lane_action_plan_seed63405` 要求的 target preflight 和 parent gate：

```bash
python3 python/train/train_sb3.py --algorithm ppo --reward-profile opening-mid-boundary-retention --train-maps caramel-workshop --train-seeds 63405 --train-seed-selection cycle --train-seconds 180 --eval-seconds 180
```
...
seed `63402` 的 target-specific guard rows 已从上述 trace 中导出：`tools/export_route_recovery_samples.py` 现在支持 `--route-recovery-filter any`、`--min-enemy-pressure-risk` 和 `--preferred-target-actions 7,3 --preferred-target-selection cycle`，可把“route_recovery 不一定为负、但已经高压贴边”的开局帧转成 repair-only supervision。首批样本位于 `harness/reports/2026-05-30_rl_stage01_seed63402_target_guard_samples_001/summary.md`，共 `17` 条，全部来自 `soda-creek` seed `63402` 的 `31.9999s-37.2664s`，原始动作均为 action `5`；由于这些帧同时贴右边和下边，action `3` 会继续顶右边，专用校验器最终只接受 action `7` 作为非顶墙目标。样本校验结论为 `edge_recovery_samples_valid`，behavior clone dry-run 也确认 `17` 条均被读取为 `edge_recovery_supervision`。但它仍只是下一轮 parent-preserving 5e-7 repair 的训练输入，不是 policy gate、stage 02 许可或 RL acceptance。

首个 seed `63402` guard-anchor probe 也未解除目标 blocker。该 run 先用 `17` 条 guard rows 训练 behavior-clone anchor，离线 train / validation accuracy 均为 `1.0`，再从 e30 mid-anchor guarded parent 做 `256` timestep `opening-boundary-escape` PPO continuation，学习率保持 `5e-7`，anchor regularization weight 为 `0.05`。结果 parent 60 / 180 / 300 秒 no-regression 通过且 `0` blockers，但 anchor validation guard 立即失败：validation KL `4.251955`、argmax agreement `0.0`；`soda-creek` seed `63402` 仍在 `37.3331s` 死亡，动作仍为 action `5` 锁定，没有 action `7` 样本。补跑 repair-probe gate 后，即使 parent no-regression 通过，训练报告内 anchor guard failure 仍会以 `2` 个 blockers 阻止该 checkpoint 进入 limited follow-up。已记录 `fail_20260530_006`，报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_guard_anchor_probe_001/summary.md`；下一轮应强化或重构局部 guard objective，而不是把当前 checkpoint 推进 stage 02。

强 guard-anchor probe 进一步证明“离线 guard rows 对齐”不是在线修复证据。该 run 把 anchor regularization weight 提到 `1.0`，每个 chunk 做 `20` 个 KL epoch，anchor lr 为 `0.001`；训练期 validation KL 降到 `0.178923`、argmax agreement 达到 `1.0`，离线 guard alignment 也通过，mean KL 为 `0.16974`、argmax agreement 为 `1.0`。但 60 秒 `soda-creek` 反而降到 `0.3333`，seed `63402` 仍在 `45.2330s` 死亡；180 秒 `soda-creek` 降到 `0.0`，300 秒三图仍全 `0.0`。相对 parent 的 window no-regression 失败并记录 `8` 个 blockers，主要集中在 `soda-creek` 60/180/300 秒和 `cracked-star-jar` 180/300 秒。已记录 `fail_20260530_007`，报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_guard_anchor_strength_probe_001/summary.md`；下一轮不能继续单纯提高 guard 强度，必须加入在线 seed `63402` preflight、成功开局 retention anchor 或 staged / constrained repair，并同时要求目标 seed 改善和 60 / 180 / 300 秒 parent preservation。

`tools/validate_policy_target_seed_preflight.py` 已提供在线目标 seed 预检：它读取已有 comparison JSON，按 `map_id:seed` 检查指定 episode 是否活到窗口、是否非 defeat、是否满足最小存活时间，以及动作比例上下限。首个真实示例读取强 guard-anchor 的 60 秒报告并检查 `soda-creek:63402`，结论为 `policy_target_seed_preflight_failed`：该 seed 只活到 `45.2330s`、终局为 `defeat`，action `7` 比例为 `0.0`，低于要求的 `0.1`。报告位于 `harness/reports/2026-05-30_rl_target_seed_preflight_validator_001/summary.md`；后续 stage 01 repair 应先跑该预检，再决定是否值得进入完整 60 / 180 / 300 秒矩阵。
...
首个 guard + retention PPO 小步 probe 仍失败：从 `ppo_e30_mid_anchor_guarded_probe.zip` 继续 `256` timestep，使用 `49` 条 opening anchor 样本、anchor weight `0.5`、`10` 个 KL epoch 与 anchor lr `0.0005` 后，anchor validation guard 在第一轮失败，validation KL `2.155758`、argmax agreement `0.6`，训练 gate 为 `trained_anchor_validation_guard_failed_not_policy_gate`。随后只跑 `soda-creek` 60 秒三 seed smoke，target preflight 抓到 seed `63402` 在 `37.4998s` defeat 且 action `7` ratio 为 `0.0`，因此没有继续完整 60 / 180 / 300 秒矩阵。报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_guard_retention_probe_001/summary.md`，失败记录为 `fail_20260530_008`。
...
首个 counterlabel + retention anchor / PPO probe 已失败：`80` 条 anchor 样本由 `48` 条 pre-failure counterlabel rows 与 `32` 条成功 retention rows 组成，action 分布为 `7 = 56/80`、`3 = 24/80`；离线 behavior-clone 诊断 accuracy 只有 `0.75`，预测 action `7/3` 为 `52/28`，没有 action `5`。从 `ppo_e30_mid_anchor_guarded_probe.zip` 继续 `256` timestep 后，anchor validation guard 仍在第一轮失败，validation KL `1.971987`、argmax agreement `0.625`；`soda-creek:63402` target preflight 仍在 `37.6664s` defeat，action `7` ratio `0.0`。报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_counterlabel_retention_probe_001/summary.md`，失败记录为 `fail_20260530_009`；下一轮不应继续 plain mixed counterlabel anchor，应改为显式 staged / constrained opening branch 或先生成更高置信 counterfactual labels。

generic staged opening wrapper 预检也已失败：评估时前 `60s` 使用既有 `stage01_corner_risk_delta_smoke.zip`，fallback 使用 `ppo_e30_mid_anchor_guarded_probe.zip`，只跑 `soda-creek` seeds `63400-63402` 的 60 秒 smoke。`soda-creek:63402` target preflight 在 `36.4665s` defeat，action `7` ratio 仍为 `0.0`，失败动作从 action `5` 转为 action `4` 主导（`906` 次 action `4`、`188` 次 action `2`）。报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_staged_opening_wrapper_preflight_001/summary.md`，失败记录为 `fail_20260530_010`；旧 opening wrapper 不能直接作为 seed `63402` repair branch，下一轮必须基于 seed-specific trace states 或显式 constrained dispatch。
...
edge recovery supervised init 已验证两种路线：`teacher_probs` 蒸馏能把 validation argmax accuracy 提到 `0.8636`，但 combined anchor mean KL 仍为 `0.674023`，edge rows argmax agreement 只有 `0.8289`，未过 alignment；`dataset_actions` 硬标签版本通过离线 alignment（combined mean KL `0.207452`、argmax agreement `0.9444`）并让 `soda-creek:63402` target preflight 通过（`60.0328s` victory），但 60 秒三 seed comparison 变成 action `7` 占 `0.9987` 的动作塌缩，seed `63400` 在 `30.1666s` defeat，gate 为 `comparison_recorded_needs_action_bias_repair`。报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_edge_recovery_supervised_dataset_actions_001/summary.md`，失败记录为 `fail_20260530_013`；下一步必须让 edge recovery 变成状态条件分派，而不是全局 opening policy。

首个 `edge_recovery_branch` 条件分派预检已把上述结论落成 evaluation-only adapter：base 仍使用 `ppo_e30_mid_anchor_guarded_probe.zip`，branch 使用 `seed63402_edge_recovery_supervised_dataset_actions.zip`，只在 `soda-creek`、`0-60s`、boundary risk 高、enemy pressure 达标且 base action 顶边时切换，并要求 branch action 不再顶边。`soda-creek` seeds `63400-63402` 的 60 秒 comparison 达到 `1.0` 胜率，`soda-creek:63402` target preflight 通过（`60.0328s` victory，action `7` ratio `0.372016`），整体 action `7` ratio 为 `0.4525`，branch usage 只有 `4/5403`，未复现 `0.9987` 全局塌缩。报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_edge_recovery_branch_preflight_001/summary.md`；该结果仍不是 RL acceptance 或 policy candidate，下一步必须扩大到 parent no-regression 和 high-pressure 60/180/300 秒多图窗口。
...
`tools/validate_policy_adapter_scope.py` 已把上述 evaluation-only adapter 的作用域复核落成可复跑门禁：它要求顶层 `policy_adapter` 存在、mode 匹配、branch usage 只出现在允许地图和时间桶、总分支比例低于阈值，并拒绝 `candidate` / `release` / `acceptance` 等过度措辞。对 high-pressure `60s` / `180s` / `300s` 三个 branch comparison 复核后，`policy_adapter_scope_passed`，共 `108848` 次决策、`18` 次 branch decision、总 branch ratio `0.000165`，所有非零分派均限于 `soda-creek` 与 `opening_lt_60`，无 errors / blockers。该结果只证明 adapter provenance 和 scope 没有扩大污染，仍不是 RL acceptance、policy candidate 或 release evidence。
...
随后执行的 chained high-pressure 诊断显示，`edge_recovery_branch + late_recovery_filter` 相对单独 `edge_recovery_branch` 通过了 60 / 180 / 300 秒 no-regression，`window_regression_vs_edge_branch.json` 为 `policy_window_regression_passed` 且 `0` blockers。60 秒平均胜率保持 `0.8889`，180 秒平均胜率保持 `0.8889` 且 `caramel-workshop` 从 `0.3333` 提升到 `0.6667`；300 秒平均胜率提升到 `0.4444`，其中 `soda-creek` 从 `0.0` 提升到 `1.0`、`cracked-star-jar` 从 `0.0` 提升到 `0.3333`，但 `caramel-workshop` 仍为 `0.0`。样本校验中 edge-only 文件均为 `edge_recovery_samples_valid`，合并 late-risk 样本 `2000` 条为 `risk_recovery_samples_valid`，但保留 `6` 条 target risk score warning。报告位于 `harness/reports/2026-05-30_rl_edge_branch_late_filter_high_pressure_001/summary.md`，失败记录为 `fail_20260530_015`；结论是该组合是强 repair diagnostic / no-regression 证据，但仍不是 policy candidate、stage 03 或 RL acceptance。

针对剩余 `caramel-workshop` 300 秒缺口的单图 trace 诊断显示失败面需要拆分，而不是继续扩大同一个 late filter。`caramel-workshop` seeds `63400-63402` 复跑仍为 `0.0` 胜率，平均存活 `163.3014s`；其中 seeds `63400` / `63401` 分别在 `213.2166s` / `235.3546s` 的 `late_180_to_300` 死于低血量 + hazard pressure，而 seed `63402` 在 `41.333s` 的 `opening_lt_60` 死于右下角 action `5` lock，且当前 edge branch 因只允许 `soda-creek` 分派而在 caramel 上触发 `0` 次。clean risk 样本拆出 `60-180s` `158` 条和 `180-300s` `100` 条，合并 `258` 条均为 `risk_recovery_samples_valid` 且 `0` warning；route hotspot 则显示 `147` 个负 `route_recovery` sampled rows 中 `100` 个集中在 opening。报告位于 `harness/reports/2026-05-30_rl_caramel_late_conversion_diagnostic_001/summary.md`，失败记录为 `fail_20260530_016`；下一轮应拆成 caramel opening escape 与 caramel late conversion 两个目标，不能把该结果解释为单纯 late filter 权重不足。

随后针对 `caramel-workshop:63402` opening lock 的 edge branch scope probe 显示，直接把 branch 放宽到 `0-60s` 虽能让目标 seed 通过 60 秒预检，却因 `300s/caramel-workshop` action `5` ratio 增加 `0.2746` 超过 `0.25` 阈值而失败。把分派窗口收窄到 `30-45s` 后，仅在 `31.2665s` 触发 1 次，把原 action `5` 改为 action `7`，目标 seed 60 秒预检通过，high-pressure `60/180/300s` 相对 parent 的 window regression 也以 `0` blockers 通过；`caramel-workshop` 60 秒胜率从 `0.6667` 到 `1.0`，180 秒从 `0.3333` 到 `0.6667`，300 秒平均存活从 `120.1944s` 到 `180.0673s`，但 300 秒胜率仍为 `0.0`。报告位于 `harness/reports/2026-05-30_rl_caramel_opening_edge_branch_probe_001/summary.md`，失败记录为 `fail_20260530_017`；结论是 caramel opening 可用极窄 evaluation-only constraint 保住，但 late conversion 仍必须单独修，不能把该 adapter 当作 policy candidate。

把 `caramel-workshop` narrow opening branch 与 late filter 串回统一 high-pressure 链路后，60 秒和 180 秒三图均达到 `1.0` 胜率，且相对上一版 chained diagnostic 的 window regression 都是 `0` blockers；但 300 秒检查失败：`soda-creek` 从 `1.0` 回落到 `0.6667`，平均存活从 `300.015s` 降到 `295.4272s`，同时 `caramel-workshop` 仍为 `0.0` 胜率，只是平均存活从 `163.3014s` 提升到 `231.2871s`。统一样本校验得到 `8` 条 valid edge samples、`2165` 条 valid raw risk samples（含 `8` 条 target risk score warning）、`1402` 条 clean mid samples 和 `727` 条 clean late samples。报告位于 `harness/reports/2026-05-30_rl_caramel_opening_late_chain_probe_001/summary.md`，失败记录为 `fail_20260530_018`；结论是该组合只能作为诊断和样本来源，下一步必须做 per-map 或更强 state-conditioned late conversion，且保留 `soda-creek` 300 秒 no-regression 为硬约束。
...
反向隔离探针保留 all-map `late_recovery_filter`，只把 `edge_recovery_branch` 限到 `caramel-workshop`。该组合保住了 `cracked-star-jar` 300 秒 `0.3333` 胜率，但 60/180/300 秒窗口全部因 `soda-creek` 回归失败：300 秒 `soda-creek` 从 `1.0` 降到 `0.3333`，平均存活下降 `92.0373s`，seed `63402` 在 `37.6664s` opening 再次死亡。`caramel-workshop` 仍为 `0.0` 胜率，平均存活 `231.2871s`。样本校验得到 `3` 条 valid edge samples、`1806` 条 valid raw risk samples（含 `6` 条 warning）、`1139` 条 clean mid samples 和 `634` 条 clean late samples。报告位于 `harness/reports/2026-05-30_rl_caramel_branch_late_all_high_pressure_001/summary.md`，失败记录为 `fail_20260530_020`；结论是上一版 soda-winning chain 同时依赖 `soda-creek` opening branch 和 all-map late safety，下一步需要显式 per-map branch composition，而不是单一共享 branch scope。

为表达上述 per-map branch composition，`train_sb3.py` 已支持 `--edge-recovery-branch-map-overrides`，格式为 `map:min_seconds:max_seconds[:min_pressure[:min_low_health_risk[:min_boundary_edge_risk]]]`。该参数会在 `policy_adapter_report` 中记录 `map_overrides` 和 `effective_map_configs`，用于同一个 evaluation-only `edge_recovery_branch` 在不同地图使用不同时间窗 / 阈值；它只用于诊断组合和样本 provenance，不是训练目标、policy candidate 或 acceptance 捷径。

使用 `soda-creek:0-60s` 与 `caramel-workshop:30-45s` 的 per-map branch composition，并保留 all-map `late_recovery_filter` 后，high-pressure 60 / 180 / 300 秒相对上一条 chained diagnostic 的 `window_regression_vs_previous_chain.json` 为 `policy_window_regression_passed`，`0` blockers。60 秒和 180 秒三图均为 `1.0` 胜率；300 秒仍为平均胜率 `0.4444`，其中 `soda-creek` 保持 `1.0`、`cracked-star-jar` 保持 `0.3333`、`caramel-workshop` 仍为 `0.0`，但 caramel 平均存活从上一条 chain 的 `163.3014s` 提升到 `231.2871s`。样本校验得到 `21` 条 valid edge samples、`2184` 条 valid raw risk samples（含 `7` 条 target risk score warning）、`1360` 条 clean mid samples 和 `784` 条 clean late samples。报告位于 `harness/reports/2026-05-30_rl_per_map_edge_branch_late_all_high_pressure_001/summary.md`，失败记录为 `fail_20260530_021`；结论是 per-map branch composition 成功保住 `soda-creek` opening 修复和 all-map late safety，但 `caramel-workshop` 仍缺少真正的 late terminal conversion objective。

为继续验证 `caramel-workshop` late terminal conversion，`train_sb3.py` 已支持 `--terminal-conversion-map-overrides`，格式为 `map:min_seconds:max_seconds[:min_pressure[:min_low_health_risk]]`；`edge_recovery_branch` 的 `policy_adapter_report` 也会展开内层 base policy adapter，避免 terminal branch 被外层 wrapper 包住后丢失 usage provenance。将旧的 `terminal_victory_multimap_path_target_w1_e30` 作为 `caramel-workshop` 专属 terminal branch 叠到 per-map edge branch + all-map late filter 基线上后，300 秒结果仍为 `soda-creek 1.0 / caramel-workshop 0.0 / cracked-star-jar 0.3333`。报告记录 terminal branch 在 `caramel-workshop` 触发 `796 / 20604` 次，其中 late bucket 为 `796 / 4404` 次、比例 `0.1807`，但没有产生胜利转换；相对 per-map chain baseline 的 `window_regression_vs_per_map_chain.json` 失败，唯一 blocker 是 `300s/caramel-workshop` 平均存活下降 `2.3338s`。报告位于 `harness/reports/2026-05-30_rl_caramel_terminal_branch_per_map_chain_001/summary.md`，失败记录为 `fail_20260530_022`；结论是当前问题不再是分支没有调用，而是旧 multimap terminal model 不会处理 caramel 真实 late failure states，下一步应收集或训练更贴近 `caramel-workshop` 210-300 秒失败态的专用 conversion target。

已基于最新 per-map chain baseline 重新导出 `caramel-workshop` seeds `63400-63402` 的 failed-only sampled traces，并切出真正对齐当前失败面的 conversion target trace pack。三条失败局分别在 `213.2166s`、`235.3546s`、`243.7898s` 死亡；`180-300s` clean risk subset 保留 `164` 条 `risk_recovery_supervision_sample` 且通过 `risk_recovery_samples_valid`，`210-300s` 失败 policy rows 导出 `387` 条 sampled states。将这些失败窗口与旧 multimap victory terminal dataset 的 same-map `210-240s` 样本做近邻对照后，发现可用 caramel victory candidate 只有 `180` 条且全部来自 seed `62301`；三个新失败 seed 的 nearest target match ratio 分别只有 `0.0`、`0.1569`、`0.169`。报告位于 `harness/reports/2026-05-30_rl_caramel_late_conversion_target_001/summary.md`；结论是旧 victory target 对最新 caramel late failure states 覆盖太薄，下一步应把 clean risk rows 作为低权重安全修复输入，同时收集更贴近 `63400-63402` 的成功 terminal trajectories，再训练新的 caramel 专属 terminal branch。

针对这条结论，已完成 `caramel-workshop` 规则 Bot terminal seed 扫描。精确目标 seeds `63400-63402` 上 9 个规则 Bot 全部 `0/3`，说明无法直接取得同 seed 成功轨迹；但扩展到邻近 seeds `63380-63420` 后，规则 Bot 共找到 `16` 个 same-map `300s` 胜利 episode，其中 `kite` `9` 局、`random` / `greedy` 各 `2` 局、`boss-hunter` / `zone-control` / `route` 各 `1` 局。用 `tools/filter_bot_trajectory_samples.py` 从这些成功局切出 `210-300s` victory movement samples，得到 `8630` 条 observation v2 样本；behavior-clone dry-run 判定 `dataset_validated_not_training_gate`，observation_len 为 `145`，action_count 为 `9`，late_low_health ratio 为 `0.9532`，无 diagnosis flags。对当前三条失败 trace 做 nearest-neighbor 检查后，match ratio 提升到 `0.5527`、`0.5605`、`0.4182`，明显优于旧 seed `62301`-only victory target。报告位于 `harness/reports/2026-05-30_rule_bot_caramel_terminal_seed_scan_001/summary.md`，失败记录为 `fail_20260530_023`；这些样本只是新的 caramel terminal branch 训练输入，不是 policy gate、stage 03 checkpoint 或 RL acceptance 证据。

基于上述 `8630` 条 same-map rule-bot victory samples 和当前 `164` 条 clean risk rows，已训练新的 `caramel-workshop` 专属 terminal branch。`distillation_report.json` 记录 victory rows 全部用 dataset action 覆写，clean risk rows 以 `top_k_scores` 软目标和 `0.25` sample weight 进入；离线 action-distribution guard 通过，validation dominant action ratio 为 `0.1887`，normalized entropy 为 `0.9189`。但嵌回当前 per-map edge branch + all-map late filter baseline 后，60/180 秒三图仍为 `1.0`，300 秒仍是 `soda-creek 1.0 / caramel-workshop 0.0 / cracked-star-jar 0.3333`；terminal branch 在 caramel 300 秒 comparison 中触发 `839` 次，仍未转胜，且 `window_regression_vs_per_map_chain.json` 因 `300s/caramel-workshop` 平均存活 `-0.2778s` 失败。报告位于 `harness/reports/2026-05-31_rl_caramel_rule_victory_terminal_branch_001/summary.md`，失败记录为 `fail_20260531_001`；结论是更近的同图胜利样本改善了离线覆盖，但简单 supervised terminal-path imitation 仍不足以解决真实 late conversion，下一轮需要更贴近失败态的成功轨迹或更强的在线 / sequence objective，并继续保留同一 `60/180/300s` no-regression 门禁。

随后把 `caramel-workshop` 规则 Bot 搜索窗口扩大到 seeds `63300-63500`，只复跑上一轮产生过胜利的 `random`、`greedy`、`kite`、`boss-hunter`、`zone-control` 和 `route`。wide scan 共找到 `85` 个 same-map `300s` 胜利 episode，并过滤出 `45853` 条 `210-300s` victory movement samples；但目标 seeds `63400-63402` 仍没有直接胜利，最近成功仍是 `boss-hunter@63399`。对三条当前失败 trace 做 nearest-neighbor 检查后，wide 样本把平均距离从 `2.528792 / 2.416173 / 2.456220` 降到 `2.401258 / 2.252513 / 2.319987`，但 target action match ratio 变成 `0.4176 / 0.5534 / 0.4915`，即 `seed63400` 明显回退、`seed63401` 基本持平、只改善 `seed63402`。报告位于 `harness/reports/2026-05-31_rule_bot_caramel_wide_terminal_seed_scan_001/summary.md`，失败记录为 `fail_20260531_002`；结论是单纯扩大 rule-bot victory 样本池会带来更近 observation，但不稳定改善可模仿 action target，下一步不应再直接训练 plain supervised terminal branch，而应转向 constrained online / sequence objective，或先做选择性轨迹加权并证明不会伤害 `63400/63401`。

为把“选择性轨迹加权”落成可审计输入，新增 `python/train/rank_behavior_clone_dataset_episodes.py`：它会按失败 trace 最近邻覆盖、action match ratio 和平均距离把 dataset episode 排名。对 wide victory 样本运行后，combined top episodes 为 `greedy@63463`、`greedy@63417`、`greedy@63489`、`greedy@63488`、`route@63333`；其中 `greedy@63463` hit 最多但 match ratio 只有 `0.6335`，而 `greedy@63417`、`greedy@63489`、`greedy@63488`、`route@63333`、`route@63412`、`route@63348` 更适合作为下一轮 selective weighting 候选。报告位于 `harness/reports/2026-05-31_rule_bot_caramel_wide_terminal_seed_scan_001/episode_ranking_wide_rule_victory.md`；该排名仍是 watch-only 诊断，不替代训练、action-distribution guard 或 `60/180/300s` no-regression。

基于 episode ranking，又用 `tools/filter_bot_trajectory_samples.py --seed ... --bot ...` 切出 selective ranked victory dataset：`greedy@63417`、`greedy@63488`、`greedy@63489`、`route@63333`、`route@63348`、`route@63412` 共 `6` 个 episode、`3237` 条 `210-300s` 样本。behavior-clone dry-run 通过 `dataset_validated_not_training_gate`，observation_len 为 `145`，action_count 为 `9`，只保留 `high_action_persistence` watch。这个小池的 nearest target match ratio 达到 `0.8509 / 0.8627 / 0.8058`，明显高于旧邻近样本和全量 wide 样本，但平均距离为 `2.550705 / 2.425806 / 2.492100`，略高于 full wide pool。报告位于 `harness/reports/2026-05-31_rule_bot_caramel_wide_terminal_seed_scan_001/selective_ranked_victory_sample_filter.md`；下一步可把它作为小型 selective weighting / branch probe 输入，但仍必须经过离线 action-distribution guard、当前 per-map edge branch + all-map late filter 的 `60/180/300s` no-regression 和 failure case 记录。

低权重 selective branch probe 已验证“离线动作一致性提升”仍不足以解决在线 terminal conversion。`w0.5` selective branch 使用 `3237` 条 ranked victory samples 和 `164` 条 clean risk rows 训练，离线 action-distribution guard 通过，overall dominant action ratio `0.1647`、normalized entropy `0.9372`；但嵌回 per-map edge branch + all-map late filter 后，`60/180s` 三图保持 `1.0`，`300s` 仍为 `soda-creek 1.0 / caramel-workshop 0.0 / cracked-star-jar 0.3333`，且 `300s/caramel-workshop` 平均存活相对 baseline 下降 `0.4223s`，repair gate 拒绝。已记录 `fail_20260531_003`；下一步不要继续做 simple supervised terminal-path imitation 的样本池/权重微调，应改为直接目标 seed 成功轨迹、constrained online / sequence objective，或先建立序列级 terminal conversion 验证。

为把“序列级 terminal conversion 验证”落成可审计门槛，新增 `tools/validate_terminal_conversion_probe.py`。该 gate 不判断 RL acceptance，只检查 terminal branch 是否在目标地图 / 时间桶真实调用、目标 `win_rate` 与存活是否相对 baseline 改善，以及配套 `window_regression` 是否通过；后续若提供 `--required-policy-adapter-scope`，还会把 `policy_adapter_scope_passed` 作为必需证据。对上述 `w0.5` selective branch 复核时，`policy_adapter_scope_terminal_300s.json` 已确认嵌套在 wrapper 内的 `terminal_conversion_branch` 只在 `caramel-workshop` / `late_180_to_300` 使用，`73851` 次决策中 terminal branch 触发 `832` 次、总 ratio `0.011266`，无 errors / blockers；但 `300s/caramel-workshop` 胜率仍为 `0.0`、相对 baseline 胜率增量 `0.0`、平均存活 `-0.4223s`，且 window regression 失败，因此 `terminal_conversion_probe_gate` 判定为 `terminal_conversion_probe_failed`。报告位于 `harness/reports/2026-05-31_rl_caramel_selective_ranked_victory_terminal_branch_w05_001/terminal_conversion_probe_gate.md`；后续 caramel terminal branch 若要进入 limited follow-up，必须同时通过 adapter scope、目标转换和 no-regression，不能只用离线 action agreement、样本 nearest match 或 branch usage 次数证明 conversion。
...
首个 `terminal-sequence-recovery` guarded closed-loop probe 从 e30 mid-anchor checkpoint 出发，只在 `caramel-workshop` seeds `63400-63402` 上继续 `256` timestep，anchor guard 通过（final validation mean KL `0.106769`、argmax agreement `0.8251`）。嵌回当前 per-map edge branch + all-map late filter 后，300 秒结果从 per-map chain 的 `soda-creek 1.0 / caramel-workshop 0.0 / cracked-star-jar 0.3333` 提升到 `1.0 / 0.3333 / 1.0`，其中 caramel 平均存活 `+17.0636s`；但 60 秒 `caramel-workshop` 从 `1.0` 回落到 `0.6667`，平均存活 `-4.1111s`，`window_regression_vs_per_map_chain` 判定失败。报告位于 `harness/reports/2026-05-31_rl_caramel_terminal_sequence_recovery_probe_001/summary.md`，失败记录为 `fail_20260531_004`；下一轮应保留该 online sequence signal，但必须加 staged opening wrapper、terminal-only branch 或训练中 60 秒 caramel hard guard，不能直接推进该 checkpoint。

两条 opening-preservation 诊断进一步缩小了问题。粗粒度 staged opening wrapper 在前 `60s` 使用 e30 mid-anchor parent，之后切回 terminal-sequence checkpoint，能把 60/180 秒三图都保持为 `1.0`，但 300 秒 caramel 又回到 `0.0`、cracked 回到 `0.3333`，且相对 per-map chain 有 `300s/caramel-workshop -1.6003s` 与 `300s/cracked-star-jar -19.3985s` 存活回归。更窄的 `caramel-workshop` 180 秒 late split 保住 60/180 秒和 soda/cracked 300 秒基线，但 caramel 仍为 `0.0`，平均存活 `-2.4116s`。报告位于 `harness/reports/2026-05-31_rl_terminal_sequence_staged_opening_probe_001/summary.md` 与 `harness/reports/2026-05-31_rl_terminal_sequence_late_split_probe_001/summary.md`，失败记录为 `fail_20260531_005`；结论是 terminal-sequence 收益依赖更早的闭环轨迹，不能用简单 0-60 wrapper 或 180+ split 隔离。下一轮应做 selective opening hard guard / seed-state dispatch / 训练中 60s caramel preflight，而不是继续调单一时间阈值。

定向 opening guard 已把上述思路落成可复跑参数：`--opening-model-targets caramel-workshop:63400` 只在指定地图 / seed 的前 `60s` 使用 e30 mid-anchor parent，其他 episode 直接使用 terminal-sequence checkpoint。嵌回当前 per-map edge branch + all-map late filter 后，60/180 秒 high-pressure 三图均保持 `1.0`，300 秒结果为 `soda-creek 1.0 / caramel-workshop 0.3333 / cracked-star-jar 1.0`；相对 per-map chain baseline，caramel 300 秒平均存活 `+17.0636s`、cracked 300 秒平均存活 `+10.2753s`，`window_regression_vs_per_map_chain` 以 `0` blocker 通过。报告位于 `harness/reports/2026-05-31_rl_terminal_sequence_selective_opening_guard_probe_001/summary.md`；该结果是 repair evidence，不是 RL test Bot、stage 03 或 acceptance。由于本轮不是 `terminal_conversion_branch` 分派，branch usage gate 不适用；如果后续回到 terminal branch 路线，仍必须单独通过 terminal conversion probe gate。
...
`train_sb3.py` 现在提供 evaluation-only `--health-retention-model` guard，可按地图或 `map:seed` target 在指定时间窗切回 baseline-winning retention model，并在 `policy_adapter_report` 中记录 target、窗口、阈值和 branch usage。该 wrapper 只用于 baseline-winning path retention 诊断和监督样本来源，不是 policy candidate、RL test Bot、stage 03 或 acceptance 捷径。

围绕 seed `63407` 的三组 retention probe 把失败面进一步收窄：`180-300s` low-health-only guard 只触发 `181 / 71589` 次，10 seed follow-up 仍为 `0.1` 胜率、平均存活 `238.65s`，`policy_window_regression_failed`；`180-300s` 全段 late path retention 也只把单 seed `63407` 从 `231.0204s` 推到 `235.6881s`，仍失败。只有 `60-300s` mid-path retention 能把单 seed `63407` 恢复为 `300.0150s` 胜利，并在 `63400-63409` 10 seed follow-up 中保留 seed `63402` 胜利、恢复 seed `63407` 胜利，使胜率从 `0.1` 提到 `0.2`、平均存活从 `242.8409s` 提到 `245.5495s`，`window_regression_caramel_300s_10seed` 以 `0` blocker 通过。报告位于 `harness/reports/2026-05-31_rl_health_retention_guard_seed63407_probe_001/summary.md`、`harness/reports/2026-05-31_rl_late_path_retention_seed63407_probe_001/summary.md`、`harness/reports/2026-05-31_rl_mid_path_retention_seed63407_followup_001/summary.md`；结论是 `63407` 的偏离发生在 `60-180s` 中窗 path retention，而不是 180 秒后低血量瞬时动作。

随后复跑 high-pressure `60/180/300s` 三图窗口，`caramel-workshop:63407` 的 target-specific mid-path retention 没有污染 seeds `63400-63402` 的既有结果：60 秒和 180 秒三图均为 `1.0` 胜率，300 秒为 `soda-creek 1.0 / caramel-workshop 0.3333 / cracked-star-jar 1.0`，相对 per-map chain baseline 的 `window_regression_vs_per_map_chain` 为 `policy_window_regression_passed`、`0` blockers。`policy_adapter_scope_health_retention.json` 进一步确认 `health_retention_guard` 在三窗口 `48603` 次决策中触发 `0` 次、ratio `0.0`，无 errors / blockers，符合 `63407` 不在 `63400-63402` high-pressure 窗口内的预期。由于该 high-pressure 窗口不包含 seed `63407`，它只证明 target scope 和 provenance 没有扩大污染；真正的 repair 信号仍来自 10 seed target follow-up。下一轮应把 `60-180s` route/path retention 转成训练期 constrained objective，而不是继续扩大 low-health-only guard。

首个真实 `mid-path-retention` closed-loop probe 已把上述信号转成训练目标：从 e30 mid-anchor parent 出发，在 `caramel-workshop:63407` 上继续 `256` timestep，anchor guard 通过（final validation mean KL `0.10678`、argmax agreement `0.8255`）。裸 SB3 10 seed caramel follow-up 相对 parent 从 `0.0` 胜率提升到 `0.1`，平均存活 `+10.5663s`，并把 seed `63407` 从 `218.5177s` defeat 转成 `300.0150s` victory；target-seed preflight 同时确认 `63402` 仍在 `40.0997s` opening defeat、`63405` 只到 `138.6673s` defeat。更重要的是，同 seed high-pressure parent-preservation 失败：`60s/caramel-workshop` 胜率 `-0.3334`，`180s/cracked-star-jar` action distribution L1 `0.4646`，`300s/caramel-workshop` action distribution L1 `0.5872`。报告位于 `harness/reports/2026-05-31_rl_mid_path_retention_seed63407_closed_loop_probe_001/summary.md`，失败记录为 `fail_20260531_007`；结论是 `mid-path-retention` 有训练信号，但单 seed continuation 不能推进为 policy candidate，下一步必须拆开 `63402` opening repair 与 `63407` mid-path retention，并加入 `60s/caramel-workshop` hard preflight / early stop。
...
`validate_policy_window_target_preflight.py` 已补上窗口级绝对阈值预检，用于在已有 comparison report 中检查某个 map/window 是否满足最低胜率、平均存活、动作熵或 dominant action ratio 等 early-stop 条件。`validate_rl_repair_probe_gate.py` 也支持 `--required-window-regression e30=...` / `parent=...`、`--required-target-seed-preflight label=...`、`--required-window-target-preflight label=...` 和 `--required-policy-adapter-scope label=...`，可把多基线 no-regression、seed 级预检、60 秒短窗 hard preflight 与 evaluation-only adapter scope 纳入同一包级门禁。后续从 limited-followup parent 继续的 RL probe 必须同时提供原始 baseline、parent-preservation regression report、相关 target seed preflight、必要的窗口 target preflight；如果包含 policy adapter / wrapper，还必须提供 adapter scope 复核。任何 parent 回归、target seed 预检失败、短窗绝对阈值失败或 adapter scope 失败都应阻止继续加长或推进。本次对 `mid-path-retention` seed `63407` closed-loop probe 的复跑给出 `rl_repair_probe_gate_failed`，集中列出 `9` 个 blockers：`3` 个 parent-preservation 回归、`2` 个 `63402` opening preflight 失败、`2` 个 `63405` retention preflight 失败，以及 `2` 个 `60s/caramel-workshop` window target preflight 失败。另一个 adapter scope smoke 证明 `edge_recovery_branch` 的 high-pressure no-regression 与 `policy_adapter_scope_passed` 可以作为 required 输入进入 repair-probe gate，但仍只允许 limited follow-up，不是 policy candidate。

`tools/create_rl_repair_split_plan.py` 已把上述 hard preflight 转成可审计拆分计划：`repair_split_plan.json` / `repair_split_plan.md` 将 `target63407` 作为已通过 required evidence 保留，并把失败面拆成 `opening63402`、`retention63405`、`short60` 和 `parent` 四条后续 lane，分别对应 target seed repair、短窗 early-stop hard preflight 与 parent-preservation。该工具只分组既有 gate 证据，不重新仿真、不批准 checkpoint；后续训练不得再用一个 shared PPO continuation 同时覆盖 `63402` opening repair 和 `63407` mid-path retention。

补充的 `repair_lane_inventory.json` / `repair_lane_inventory.md` 说明：`opening63402` 可复用 `caramel-workshop` narrow opening branch 和 selective opening guard 作为诊断证据，但它们仍是 evaluation-only 且曾引入 seed tradeoff；`retention63405` 目前没有正向修复证据，已先按 lane 补做 parent-vs-candidate trace compare；`short60` 与 `parent` 继续作为 required hard gate，而不是可在局部 target seed 改善后跳过的附属检查。

seed `63405` 的 parent-vs-candidate trace compare 和 route hotspot 进一步证明，该 blocker 不是纯 late low-health 问题：candidate 比 parent 早死 `75.8496s`，首次动作分歧在 `13.0001s`（action `3` -> action `7`），首次 high-pressure 从 `100.9988s` 提前到 `45.9996s`；负 route_recovery 热点共 `248` 条，主要集中在 `opening_lt_60` 与 `mid_60_to_180` 的 `boundary_edge` 压力。因此下一步应把 `retention63405` 作为 opening + mid-window boundary/path retention lane 单独设计修复目标，并要求 seed `63405` retention preflight、`60s/caramel-workshop` hard preflight 与 parent no-regression 同时通过，不能直接并入 `63407` shared PPO continuation。

`tools/create_rl_lane_repair_action_plan.py` 已把 lane inventory、trace compare 和 route hotspot 汇总为结构化 action plan。当前 `lane_action_plan_seed63405.json` 判定 `retention63405` 为 `opening_mid_boundary_path_retention`：objective scope 必须覆盖 `opening_lt_60` boundary escape / route retention 与 `mid_60_to_180` boundary/path retention；禁止纯 late low-health continuation、与 `target63407` 混合的 shared PPO continuation，以及放宽 `short60` 或 parent-preservation gate。该 plan 只是 repair routing，不是 policy candidate、stage 03 或 RL acceptance。

为该 lane 新增的 `opening-mid-boundary-retention` reward profile 已接入 Rust `gym-bridge`、Python Gym wrapper 与 `train_sb3.py` CLI，并通过单测和 `caramel-workshop:63405` dry-run。该 profile 在 `0-180s` 放大 boundary/path retention 相关信号，`180s` 后关闭；0-60 秒额外保留 opening boundary escape，因此适合作为 seed `63405` lane-specific 小步 probe 的训练入口，但仍必须绑定 `lane_action_plan_seed63405` 的全部门禁。

首个 seed `63405` lane-specific 小步 probe 已证明 profile 有局部信号但不能继续加长。该 run 从 e30 mid-anchor parent 出发，只在 `caramel-workshop:63405` 上用 `opening-mid-boundary-retention` 训练 `256` timesteps，anchor guard 通过（final validation mean KL `0.106725`、argmax agreement `0.8258`）。seed `63405` 从上一轮 `138.6673s` defeat 提升到 `220.6515s` defeat，10 seed caramel follow-up 相对 parent 也通过 no-regression（win rate `+0.1`、average survival `+19.2282s`）；但 hard gate 仍失败：seed `63405` 仍为 defeat，`60s/caramel-workshop` 仍只有 `0.3333` 胜率 / `49.2774s` 平均存活，high-pressure parent no-regression 还有 3 个 blockers（`180s/cracked-star-jar` action L1 `0.7526`、`300s/caramel-workshop` action L1 `0.6056`、`60s/caramel-workshop` win rate delta `-0.3334`）。已记录 `fail_20260531_008`；下一步不得从该 checkpoint 继续堆 timesteps，应把 short-window opening preservation、seed `63402` opening repair 与 seed `63405` late terminal survival / conversion 分开处理。

`create_rl_repair_split_plan.py` 现在会把 repair gate 中的 `failure_analysis.total_failures` 也作为独立 lane 输出，避免剩余失败局只停留在 warning。对 seed `63405` 小步 probe 复跑后，`repair_split_plan.md` 将 `caramel10` 标为 passed evidence，并把失败面拆成 `retention63405`（目标 seed 仍 defeat）、`short60`（60 秒 caramel 绝对阈值失败）、`parent`（high-pressure parent-preservation 失败）和 `failure_analysis`（9 个剩余 caramel-workshop 失败局）四条 lane。该 split plan 仍只是路由证据，不批准加长训练；下一步应先对 `failure_analysis` lane 拆 opening / late trace，再选择更窄的 state-conditioned branch 或 staged gate。

`analyze_rl_policy_failures.py` 已进一步输出 `failure_lanes`，把同一个 `failure_analysis` lane 拆成可执行的子目标。当前 `failure_analysis_caramel_300s_10seed.md` 显示 `opening_repair` 只有 seed `63402`（`40.0997s` opening defeat），而 `late_terminal_survival_conversion` 覆盖 seeds `63400/63401/63403/63404/63405/63406/63408/63409`，平均失败时间 `216.6965s`。因此下一步若继续 RL 修复，应把 caramel opening branch 和 late terminal conversion 分开取样、分开 preflight，不再把 9 个失败局当成同一种错误。

`create_rl_failure_lane_trace_plan.py` 已把上述 failure lanes 转成 trace/preflight 调度计划。当前 `failure_lane_trace_plan.md` 生成两条 priority 1 任务：`opening_repair` 只采 seed `63402` 的 `0-60s` opening trace；`late_terminal_survival_conversion` 需要扫 `63400-63409` 连续区间以覆盖 8 个 late seeds，并显式标出额外扫到的 `63402/63407`。该 plan 只批准采样和诊断，不批准训练；后续应先补 trace、route hotspot 与目标 preflight，再决定是否做 state-conditioned terminal branch。

已补跑上述两条 failure lane trace，并记录 `failure_lane_trace_summary.md`。`opening_repair` 的 seed `63402` 仍在 `40.0997s` defeat，`82` 个 sampled rows 中有 `51` 个负 `route_recovery`（`62.20%`），全部处于 `opening_lt_60`，压力主要是 `boundary_edge`，动作集中在 `5` / `4` / `2`，因此应作为独立 opening boundary/action-lock lane 做窄 state-conditioned branch 或 target preflight。`late_terminal_survival_conversion` 过滤掉额外扫描 seed 后有 `8` 条真实 late failure trace，`3479` 个 sampled rows 中 `2599` 个负 `route_recovery`（`74.71%`）；其中 `mid_60_to_180` 有 `1505` 个热点、`opening_lt_60` 有 `573` 个、`late_180_to_300` 有 `521` 个，`boundary_edge` 占 `2482` 个热点。结论是 late death 前已有 opening/mid route debt，下一轮不能只做纯 late low-health 或 terminal branch，必须同时保护 `60-180s` path retention、`180-300s` terminal conversion、`60s/caramel-workshop` hard preflight 和 high-pressure parent no-regression；该结果仍只是 repair routing evidence，不是 policy acceptance、RL test Bot 或 stage 03 许可。

`create_rl_failure_lane_repair_preflight.py` 已把 trace 诊断转成下一轮修复前的硬门槛契约。`failure_lane_repair_preflight.md` 要求 `opening_repair` 先走 seed `63402` target preflight、`60s/caramel-workshop` window target preflight 和 high-pressure `60/180/300s` parent no-regression；`late_terminal_survival_conversion` 则要求 `63400-63409` 10 seed caramel follow-up、同一个 short60 hard preflight 和 parent no-regression，并显式阻止纯 `180s+` low-health continuation、忽略 `60-180s` path retention 的 terminal branch，以及把未过滤连续 seed hotspot 当作 late-only 证据。该 preflight 只是 repair contract，不运行仿真、不训练 policy，也不解除 acceptance 阻塞。

opening_repair 的首轮 edge-recovery branch 预检给出混合结论。以 seed `63405` 的 opening-mid-boundary-retention probe 为 base、历史 seed `63402` edge-recovery supervised branch 为分支时，`30-45s`、`30-50s`、`30-55s` 和 `30-60s` 窄窗口都能让 `caramel-workshop:63402` 到达 `60.0328s` victory，但 target preflight 因 action `5` 比例 `0.308162 > 0.300000` 失败；扩大到 `0-60s` 后 target preflight 通过（action `5=0.112715`、action `7=0.556358`），且 60 秒 high-pressure smoke 的 `caramel-workshop` 窗口预检通过（win rate `0.6667`、平均存活 `55.9217s`）。但 broad branch 仍只跑过 60 秒 smoke，尚未通过 `180/300s` parent no-regression 或 `63400-63409` 10 seed follow-up，并且 action `7` 主导偏强。报告位于 `harness/reports/2026-05-31_rl_failure_lane_opening_repair_branch_preflight_001/summary.md`，失败记录为 `fail_20260531_009`；该结果只能作为 opening lane repair diagnostic，不是 policy candidate、stage 03 或 RL acceptance。

补跑 `0-60s` broad branch 的 high-pressure `180/300s` 固定窗口后，full parent no-regression 失败：相对未加 branch 的 base，`60s/caramel-workshop` 仍是通过项（胜率 `+0.3334`、平均存活 `+6.6443s`、action L1 `0.2031`），但 `180s/caramel-workshop` action L1 为 `0.4813 > 0.45`，`300s/caramel-workshop` action L1 为 `0.5787 > 0.45`；主要漂移是 action `5` 在 180 秒 `+0.1270`、300 秒 `+0.1940`。报告位于 `window_regression_high_pressure_window0_60_vs_base.md`，失败记录为 `fail_20260531_010`。因此 broad branch 不应进入 10 seed follow-up，下一步要回到更窄的 state-conditioned dispatch 或在线 action-distribution guard。

追加的 drift trace 诊断确认了该 blocker 的触发形态：`0-60s` broad branch 在 `caramel-workshop` seeds `63400-63402` 的 300 秒单图复跑中只触发 `3 / 19433` 次，全部发生在 `6.6333s-20.0s` 的 opening；seed `63400` 完全不变，seed `63402` 被 1 次 `5 -> 7` 分派从 `40.0997s` opening death 拉到 `212.0497s` late defeat，但 seed `63401` 的 2 次分派把后续 sampled action `5` ratio 推高 `+0.5199`，形成长窗 action `5` 吸附。报告位于 `harness/reports/2026-05-31_rl_failure_lane_opening_branch_drift_diagnostic_001/summary.md`，失败记录为 `fail_20260531_011`；下一轮 opening_repair 应排除 `63401` 这种早期 top-edge / low-moderate pressure dispatch，或先加在线 action-distribution / path-retention guard，再重新跑 target seed、short60 和 parent no-regression。

基于上述 trace，把 dispatch 收窄到 `19.0-19.8s`、`min_pressure=0.2`、`min_boundary_edge_risk=0.75` 后，micro-window branch 通过了完整 opening_repair follow-up：`caramel-workshop:63402` target seed 到达 `60.0328s` victory（action `5=0.112715`、`7=0.556358`），`60s/caramel-workshop` hard preflight 为 `0.6667` 胜率 / `55.9217s` 平均存活；high-pressure `60/180/300s` parent no-regression 为 `0` blockers，caramel 的 action L1 分别为 `0.31` / `0.1817` / `0.1889`，不再复现 broad branch 的 `180/300s` 长窗漂移；adapter scope 只记录 `3` 次 branch decision、总 ratio `0.000027`，均位于 `opening_lt_60`；`63400-63409` caramel 300 秒 follow-up 也通过，胜率维持 `0.1`、平均存活 `+17.195s`、action L1 `0.037`。报告位于 `harness/reports/2026-05-31_rl_opening_micro_window_branch_probe_001/summary.md`；该结果是 opening lane 的 repair evidence 和 limited follow-up 证据，不是 RL test Bot、stage 03 或 acceptance。下一步应把这个 evaluation-only micro dispatch 转成真实 constrained policy / branch objective，或作为下一轮 repair probe gate bundle 的固定证据输入，同时继续把 late_terminal_survival_conversion 作为独立 lane 处理。
...
terminal branch usage instrumentation 的真实 300 秒报告已补做 `terminal_conversion_branch` 作用域复核：`policy_adapter_scope_terminal_300s.json` 使用 `terminal_decisions` / `terminal_ratio` 作为计数键，确认 `44904` 次决策中 `2996` 次 terminal branch 只出现在 `late_180_to_300`，总 ratio `0.06672`，无 errors / blockers。该结果只证明终局分支确实在允许时间桶内调用，不能改变 300 秒三图 `0.0/0.0/0.0` 和仍需 per-map terminal objective 的结论。
