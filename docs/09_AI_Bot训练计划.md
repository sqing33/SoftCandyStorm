...
`validate_policy_window_target_preflight.py` 已补上窗口级绝对阈值预检，用于在已有 comparison report 中检查某个 map/window 是否满足最低胜率、平均存活、动作熵或 dominant action ratio 等 early-stop 条件。`validate_rl_repair_probe_gate.py` 也支持 `--required-window-regression e30=...` / `parent=...`、`--required-target-seed-preflight label=...` 和 `--required-window-target-preflight label=...`，可把多基线 no-regression、seed 级预检与 60 秒短窗 hard preflight 纳入同一包级门禁。后续从 limited-followup parent 继续的 RL probe 必须同时提供原始 baseline、parent-preservation regression report、相关 target seed preflight 和必要的窗口 target preflight；任何 parent 回归、target seed 预检失败或短窗绝对阈值失败都应阻止继续加长或推进。本次对 `mid-path-retention` seed `63407` closed-loop probe 的复跑给出 `rl_repair_probe_gate_failed`，集中列出 `9` 个 blockers：`3` 个 parent-preservation 回归、`2` 个 `63402` opening preflight 失败、`2` 个 `63405` retention preflight 失败，以及 `2` 个 `60s/caramel-workshop` window target preflight 失败。
