...
补充的 `repair_lane_inventory.json` / `repair_lane_inventory.md` 说明：`opening63402` 可复用 `caramel-workshop` narrow opening branch 和 selective opening guard 作为诊断证据，但它们仍是 evaluation-only 且曾引入 seed tradeoff；`retention63405` 目前没有正向修复证据，下一步应先做 parent-vs-candidate trace compare；`short60` 与 `parent` 继续作为 required hard gate，而不是可在局部 target seed 改善后跳过的附属检查。
