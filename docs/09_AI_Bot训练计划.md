...
`validate_rl_repair_probe_gate.py` 已支持 `--required-window-regression e30=...` / `parent=...` 的多基线硬门禁写法，也支持 `--required-target-seed-preflight label=...` 把 opening / retention / target conversion 的 seed 级预检纳入同一包级门禁。后续从 limited-followup parent 继续的 RL probe 必须同时提供原始 baseline、parent-preservation regression report 和相关 target seed preflight；任何 parent 回归或 target seed 预检失败都应阻止继续加长或推进。本次对 `mid-path-retention` seed `63407` closed-loop probe 的复跑给出 `rl_repair_probe_gate_failed`，集中列出 `7` 个 blockers：`3` 个 parent-preservation 回归、`2` 个 `63402` opening preflight 失败和 `2` 个 `63405` retention preflight 失败。
