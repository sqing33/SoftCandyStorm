...
首个 seed `63402` guard-anchor probe 也未解除目标 blocker。该 run 先用 `17` 条 guard rows 训练 behavior-clone anchor，离线 train / validation accuracy 均为 `1.0`，再从 e30 mid-anchor guarded parent 做 `256` timestep `opening-boundary-escape` PPO continuation，学习率保持 `5e-7`，anchor regularization weight 为 `0.05`。结果 parent 60 / 180 / 300 秒 no-regression 通过且 `0` blockers，但 anchor validation guard 立即失败：validation KL `4.251955`、argmax agreement `0.0`；`soda-creek` seed `63402` 仍在 `37.3331s` 死亡，动作仍为 action `5` 锁定，没有 action `7` 样本。已记录 `fail_20260530_006`，报告位于 `harness/reports/2026-05-30_rl_stage01_seed63402_guard_anchor_probe_001/summary.md`；下一轮应强化或重构局部 guard objective，而不是把当前 checkpoint 推进 stage 02。
