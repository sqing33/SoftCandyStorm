...
Gym reward breakdown 已新增 `opening_edge_risk_delta`，用于在 opening 阶段奖励降低“贴边 + 敌压”的状态风险；它和 `corner_risk_delta` 一样是 state-based delta shaping，不绑定具体动作，目标是修复 seed `62405` 这类沿右边界进入角落的回归，同时避免重演 action-specific 惩罚导致的动作塌缩。该改动仍需通过 60 秒 high-pressure 10 seed opening gate 验证。
