...
### P0 `local_binary_launch_blocked`

Newly built local binaries cannot be trusted as executable evidence yet.
...
- 恢复本机新编译 Mach-O 可执行文件启动能力。
- 恢复后先运行 cargo test --workspace 和 game_harness simulate quick。
- 恢复 Runtime 启动后运行 demo capture 和 9 局人工试玩审查。
- 恢复二进制后重跑 all Bot matrix 与 strict replay-batch。
- 继续减少 progress 无 report warning。
- 恢复二进制后推进 full pack 到 validate-candidates / simulate-candidates。

## Blockers

| Severity | Category | Blocker | Status | Source |
|---|---|---|---|---|
| `P0` | `host_environment` | `local_binary_launch_blocked` | `local_binary_launch_blocked` | `harness/reports/2026-05-26_local_binary_launch_diagnostic_001/local_binary_launch_diagnostic.json` |
...
| `P1` | `release` | `release_candidate_not_ready` | `release_candidate_not_ready` | `harness/reports/2026-05-26_release_candidate_evidence_current_local_004/release_candidate_evidence.json` |
| `P1` | `release` | `release_package_not_ready` | `release_package_not_ready` | `harness/reports/2026-05-26_release_package_manifest_current_local_002/release_package_manifest.json` |
