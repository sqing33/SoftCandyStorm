# Runtime Meta Tab Click Zone

- Decision: `runtime_meta_tab_click_zone_valid`
- Scope: `game_runtime` global F1-F5 meta-panel tab segmented pointer navigation zone

## Implemented

- F1-F5 局外面板新增共享页签点击提示行，覆盖概览、章节、图鉴、设置和巡逻准备页。
- 鼠标左键点击右侧面板顶部页签区时，从左到右映射为概览、章节、图鉴、设置和巡逻准备页。
- 点击入口复用 `select_runtime_meta_panel`，继续写回 `base_ui_state.selected_panel`，并保留切换面板取消删除确认态的路径。
- 页签点击切换后会结束当前帧，避免同一次点击被切换后的页面继续消费为页内操作。
- Runtime surface contract 和基地 UI 人工审查模板已同步记录该入口。

## Verification

- `cargo test -p game_runtime`: 85 passed
- `cargo test --workspace`: 150 passed
