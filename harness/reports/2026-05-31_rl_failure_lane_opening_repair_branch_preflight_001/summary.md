...
- Decision: `repair_only_opening_branch_preflight_mixed`
- Lane: `opening_repair`
...
The broad `0-60s` window is the first positive short-window diagnostic for this lane: it passes both `caramel-workshop:63402` target seed preflight and the `60s/caramel-workshop` hard preflight. However, it is not policy acceptance evidence. It has only been checked at 60 seconds, action `7` dominance is strong on the target map, and the required `180s` / `300s` high-pressure parent no-regression and 10 seed caramel follow-up are still missing.

## Blockers

- `30-45s`, `30-50s`, `30-55s`, and `30-60s` target seed preflights fail on action `5` ratio `0.308162 > 0.300000`.
- `0-60s` broad branch has not passed full `60/180/300s` high-pressure parent no-regression.
...
Do not continue with a shared PPO continuation from this checkpoint. The next step should either:

- run full parent no-regression for the `0-60s` broad branch to confirm whether its short-window win survives `180s` and `300s`, or
- design a narrower state-conditioned dispatch that keeps the `0-60s` action `5` reduction while avoiding broad action `7` dominance.
