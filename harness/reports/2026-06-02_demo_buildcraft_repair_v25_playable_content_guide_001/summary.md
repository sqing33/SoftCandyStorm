...
| 糖霜草地 (`frosting-grassland`) | 600 | 6 | 暴走搅糖机 (`runaway-sugar-mixer`) @ 210s | 蹦蹦软糖 (`bouncy-gummy`) | 蹦蹦软糖 (`bouncy-gummy`), 酸酸软糖 (`sour-gummy`), 夹心饼怪 (`sandwich-cookie-creep`), 焦糖史莱姆 (`caramel-slime`), 粘粘熊糖 (`sticky-bear-gummy`), 汽水泡泡 (`soda-bubble`), 甘草跳跳 (`licorice-skipper`), 糖粉飞蛾 (`sugar-moth`), 太妃盾糖 (`taffy-shieldling`), 糖针吐吐 (`sprinkle-spitter`) |
