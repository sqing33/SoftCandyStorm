...
|---|---:|---:|---:|---:|---:|
| DQN policy | 100% | 5.0333 | 1.0 | 3.0 | 0.0 |
...
- DQN policy 在 302 个 step 中动作 `5` 占比 `93.05%`、动作 `0` 占比 `6.95%`，触发 `dominant_action_bias` 和 `low_action_entropy`。
- 平均奖励仍由 `terminal=5.0` 主导，触发 `terminal_reward_dominance`。
...
- Gate 结论：`comparison_recorded_needs_action_bias_repair`。

## 后续

- 修复或约束 DQN 训练的动作塌缩问题，至少记录动作熵、动作分布和奖励拆解。
- 再进行更长时长、更多 seed 的 RL policy 对比。
- PPO 冒烟已经验证第二条 SB3 算法链路，但同样出现单动作塌缩，不能替代动作偏置修复。
