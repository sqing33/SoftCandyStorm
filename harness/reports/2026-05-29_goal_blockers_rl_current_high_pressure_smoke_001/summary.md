...
- 后续若 GameCore、Gym bridge 或当前 RL checkpoint 发生实质变化，再重跑 high-pressure 60/180/300 秒三窗口对比并记录新的 failure case 或解除依据。
- 准备 9 局人工试玩审查，并让真人填写 runtime_manual_review_pack。
- 准备 full pack 人工设计审查与 9 局人工试玩记录。
- 真人审查和试玩通过后，再用 acceptance manifest / lockfile 推进 accepted_content。
- 收集人工设计审查与试玩记录，并运行 strict acceptance 校验。
- 通过人工接受后再生成非空 accepted content lockfile。
...
| `P1` | `release` | `release_candidate_not_ready` | `release_candidate_not_ready` | `harness/reports/2026-05-29_release_candidate_evidence_rl_current_high_pressure_smoke_001/release_candidate_evidence.json` |
| `P1` | `release` | `release_package_not_ready` | `release_package_not_ready` | `harness/reports/2026-05-29_release_package_manifest_rl_current_high_pressure_smoke_001/release_package_manifest.json` |
