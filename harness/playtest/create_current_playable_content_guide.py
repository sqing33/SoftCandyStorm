#!/usr/bin/env python3
"""Create a human-readable playable content guide for the current candidate."""
...
from play_current_candidate import build_quick_play_command
...
def summarize_build_routes(records: dict[str, dict[str, dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    for evolution in records["evolutions"].values():
        ...
        role = budget.get("role") if isinstance(budget.get("role"), str) else ""
        routes.append(
            {
                "id": evolution["id"],
...
def build_guide(repo_root: Path, content_dir: Path = CONTENT_DIR) -> dict[str, Any]:
    ...
    return {
        ...
        "quick_play_presets": [
            {
                ...
                "command": f"python3 harness/playtest/play_current_candidate.py {preset.preset_id}",
        ...
        "content_tour_runs": [
            {
                ...
                "command": f"python3 harness/playtest/run_current_content_tour.py {run.run_id}",
        ...
        "limitations": [
            ...
            f"{CANDIDATE_LABEL} must remain out of content/base_demo and Runtime official content until human gates pass.",
        ],
    }
...
def write_markdown(guide: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        f"# {guide['candidate_label']} 可玩内容导览",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create the current playable content guide.")
