#!/usr/bin/env python3
"""Check local evidence status for the v25 manual playtest matrix."""
...
def build_report(repo_root: Path, draft_path: Path = DEFAULT_DRAFT) -> dict[str, Any]:
    ...
    if draft.get("candidate_id") != CANDIDATE_ID:
        errors.append("review draft candidate_id does not match v25 candidate")
    if draft.get("content_hash") != CONTENT_HASH:
        errors.append("review draft content_hash does not match v25 content hash")
    ...
    return {
        ...
        "summary": {
            ...
            "existing_report_count": len(RUNS) - len(missing_reports),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    for run in report["runs"]:
        lines.append(
            f"| `{run['run_id']}` | `{run['skill']}` | {run['seed']} | "
            f"`{run['report_exists']}` | `{run['draft_has_todo']}` |"
        )

    lines.extend(["", "## Missing Reports", ""])
    if report["missing_reports"]:
        lines.extend(f"- `{path}`" for path in report["missing_reports"])
    else:
        lines.append("- None")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check v25 manual playtest local evidence status.")
