#!/usr/bin/env python3
"""Audit playable-content coverage for the current candidate."""
...
from create_current_playable_content_guide import CATEGORIES, build_guide, collect_records
from current_candidate import CANDIDATE_ID, CANDIDATE_LABEL, CONTENT_DIR, CONTENT_HASH
...
def audit_candidate_state(guide: dict[str, Any], items: list[dict[str, Any]]) -> None:
    ...
    if state.get("candidate_only") is not True:
        add_action(
            ...
            action=f"检查 manifest，确保 {CANDIDATE_LABEL} 仍停留在 generated_candidates 候选池。",
        )
...
def build_audit(repo_root: Path, content_dir: Path = CONTENT_DIR) -> dict[str, Any]:
    ...
    decision = "playable_content_coverage_invalid"
    if hard_error_count == 0:
        decision = (
            "playable_content_coverage_needs_content_repair"
            if items
            else "playable_content_coverage_ready_for_human_playtest"
        )

    playable_sets = collect_playable_sets(guide)
    return {
        "report_version": 1,
        ...
        "coverage": {
            ...
            "quick_play_characters": sorted(playable_sets["quick_characters"]),
            "quick_play_maps": sorted(playable_sets["quick_maps"]),
            "content_tour_characters": sorted(playable_sets["tour_characters"]),
            "content_tour_maps": sorted(playable_sets["tour_maps"]),
        },
        "action_items": items,
        ...
        "limitations": [
            ...
            f"{CANDIDATE_LABEL} 仍是 generated candidate，不能复制到 content/base_demo、accepted_content 或 Runtime 正式内容目录。",
            "AI/RL 训练线不在本审计范围内。",
        ],
    }
...
def write_markdown(audit: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        f"# {audit['candidate_label']} 可玩内容覆盖审计",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit current playable-content coverage.")
    ...
    if audit["decision"] == "playable_content_coverage_ready_for_human_playtest":
        return 0
    if args.allow_repair and audit["decision"] == "playable_content_coverage_needs_content_repair":
        return 0
    return 1
