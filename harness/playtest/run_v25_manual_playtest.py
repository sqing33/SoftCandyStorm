...
SUMMARY_SCRIPT = Path("harness/playtest/summarize_v25_manual_playtest_reports.py")
...
def run_playtest_and_maybe_summarize(
    command: list[str],
    *,
    repo_root: Path,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-summary-after",
        action="store_true",
        help="Do not refresh the objective manual report summary after Runtime exits",
    )
...
def main() -> int:
    ...
    if args.dry_run:
        return 0
    return run_playtest_and_maybe_summarize(
        command,
        repo_root=repo_root,
