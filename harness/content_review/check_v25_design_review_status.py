#!/usr/bin/env python3
"""Check local evidence status for the v25 content design review draft."""
...
CANDIDATE_PATH = Path("harness/generated_candidates/2026-06-02_demo_buildcraft_repair_v25_full_pack")
DEFAULT_DRAFT = Path("harness/content_review/drafts/2026-06-02_demo_buildcraft_repair_v25_full_pack_design_review_draft.json")
...
def build_report(repo_root: Path, draft_path: Path = DEFAULT_DRAFT) -> dict[str, Any]:
    ...
    if draft.get("candidate_pack_id") != CANDIDATE_ID:
        errors.append("review draft candidate_pack_id does not match v25 candidate")
    if draft.get("candidate_pack_path") != str(CANDIDATE_PATH):
        errors.append("review draft candidate_pack_path does not match v25 candidate")
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# v25 Content Design Review Status",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check v25 content design-review draft status.")
