#!/usr/bin/env python3
"""Validate asset Runtime preview review records.

This gate checks the human Runtime preview review that sits between an
asset-runtime-candidate manifest and final asset acceptance. It does not load
asset files, integrate Runtime content, or approve release readiness.
"""
...
ALLOWED_DECISIONS = {"runtime_preview_pass", "needs_more_review", "repair", "reject"}
PASS_DECISION = "runtime_preview_pass"
REQUIRED_CHECKS = {
...
def validate_checks(payload: dict[str, Any], decision: Any, errors: list[str]) -> None:
    ...
    if decision == PASS_DECISION and any(checks.get(field) is not expected for field, expected in REQUIRED_CHECKS.items()):
        errors.append("runtime_preview_pass requires every required check to match")
...
def build_report(review_path: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if decision == PASS_DECISION and unresolved:
        errors.append("runtime_preview_pass requires unresolved_issues to be empty")
    ...
    return {
        ...
        "decision": "asset_runtime_preview_review_valid" if not errors else "asset_runtime_preview_review_invalid",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Soft Candy Storm asset Runtime preview reviews.")
    parser.add_argument("review", type=Path, help="Asset Runtime preview review JSON")
    ...
    return 0 if report["decision"] == "asset_runtime_preview_review_valid" else 1
