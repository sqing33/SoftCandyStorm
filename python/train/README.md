...
Use `--reward-profile late-survival` for closed-loop late-window survival repair experiments. This profile is applied inside the Rust `gym-bridge`: after 180 seconds it ramps up survival, safety-risk reduction, low-health, boundary, enemy, hazard, boss-pressure, and terminal survival rewards/penalties. Use `--reward-profile long-run-retention` when a late-window repair regresses 60-180 second retention or collapses into repeated actions; it starts ramping after 60 seconds and also amplifies the repeated-action penalty. Use `--reward-profile mid-path-retention` for bounded 60-180 second handoff/path-retention repair experiments; it ramps from 60-90 seconds, stays active through 180 seconds, and turns off afterward so it does not become another late-window conversion objective. Use `--reward-profile opening-mid-boundary-retention` for 0-180 second boundary/path-retention repair experiments that need opening boundary escape and mid-window route-retention signals in one lane, such as seed 63405. Use `--reward-profile late-win-conversion` when the policy reaches late windows but fails to convert 240-300 second states into victories; it ramps only in the final minute and gives a stronger terminal win/loss signal. Use `--reward-profile terminal-sequence-recovery` when the policy already has high nearest-action agreement in 210-300 second target traces but still dies to low-health, hazard, or boss pressure; it ramps from 180-210 seconds and keeps the stronger sequence recovery objective active through the whole terminal window. Use `--reward-profile opening-route-recovery` for 0-60 second opening repair experiments that need stronger route-recovery, safety-risk, low-health, boundary, enemy-pressure, and action-repeat signals. Use `--reward-profile opening-boundary-escape` for 0-60 second opening repair experiments that should keep the same opening route-recovery signals while adding a small pressure-gated reward for moving back toward the map interior when boundary edge risk and enemy pressure are both high. These profiles are training evidence only; deterministic high-pressure 60/180/300-second gates are still required before any RL test Bot candidate review.
...
Use `--terminal-conversion-model <zip>` with `--terminal-conversion-maps`, `--terminal-conversion-min-seconds`, and `--terminal-conversion-max-seconds` to test an explicit terminal-window branch. Reports include `policy_adapter.usage`, which records how many decisions used the base policy vs. the terminal policy overall, by map, and by time bucket; multi-map comparison reports aggregate this under `usage_scope = multimap_aggregate`. This is branch diagnostics only; a high terminal usage ratio is not acceptance evidence and still requires parent/e30 fixed-window no-regression and repair-probe gates.
...
Use `--dataset-action-target-path <PATH>` to replace teacher targets with dataset action labels for matching samples. Add `--dataset-action-target-maps <map-id,...>` when the same JSONL contains multiple maps but only one map should receive hard dataset-action targets for a per-map terminal branch; path-matched samples from other maps keep the selected teacher/recovery target mode and are counted as `map_scoped_out_sample_count`.
