...
cargo run -p game_runtime -- --accepted-lock-file harness/accepted_content/accepted_content.lock.json --accepted-content-id <candidate-id> --seed 12345 --seconds 600
...
`game_runtime --accepted-lock-file` 会从 `accepted_content` 版本锁定清单中选择已锁定内容；如果 lockfile 有多个 entry，需要传 `--accepted-content-id`，未锁定候选不会被该入口读取。
`game_runtime` 右侧会展示最小“糖罐守护站”局外面板；本局结束后会用 `game_core::meta` 结算糖晶碎片、星片、图鉴更新、章节目标和新解锁，重开局会保留本次会话内的局外进度。
